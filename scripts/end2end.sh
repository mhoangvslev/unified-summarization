if [ ! $# -eq 4 ]; then
    ...
    echo "sh script/end2end.sh <MODE> <path/to/data> <EXP_NAME> <WITHOUT_LOSS>"
    exit 1
fi

if [ ! "$4" = "True" -a ! "$4" = "False" ]; then
    echo "<WITHOUT_LOSS> must be either True or False"
    exit 1
fi
...
INCONSISTENT_TOPK=3
SELECTOR_PATH="log/selector/$EXP_NAME/eval/bestmodel-xxxx"
...
START_EVAL=1000
SINGLE_PASS=$INCONSISTENT_LOSS  # if evaluating by loss, change singel_pass to False
